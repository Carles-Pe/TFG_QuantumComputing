...
class TFG_SimplePowerModel(TFG_QuadraticModel):
    ...
    def generate_quadratic_model(self, 
        ...
        mdl = Model(name="SimplePowerModel")
        n = self.graph.number_of_nodes()
        E = self.graph.number_of_edges()
        x = {(i, p): mdl.binary_var(name=f"x_{i}_{p}") for i in range(n) for p in range(self.P)}
        ...
        # Objective function
        C_balanced = mdl.sum((mdl.sum(x[i, p] for i in range(n)))**2 for p in range(self.P))

        # C_discount, add cost to cutting edges
        C_discount = mdl.sum(E - mdl.sum(x[i, p] * x[j, p] for i, j in self.graph.edges) for p in range(self.P))
        # C_discount = E-mdl.sum(2*x[i] * x[j] -x[j] -x[i] for i, j in self.graph.edges)

        # Constraints
        P_one_hot = mdl.sum((mdl.sum(x[i, p] for p in range(self.P)) - 1)**2 for i in range(n))
        ...
        P_surplus = mdl.sum((mdl.sum(x[i, p]*(weights[i]-k) for i in range(n)))**2 for p in range(self.P))
        print("[*] No surplus constraint has been used")
        # Add the objective function to the model
        print("ALPHA:", self.alpha)
        print("C_BALANCED:",C_balanced)
        print("BETA:", self.beta)
        print("C_DISCOUNT:", C_discount)
        mdl.minimize(self.alpha * C_balanced + self.beta * C_discount + lambda_one_hot*P_one_hot + lambda_surplus*P_surplus)
        print("[*] Parabolic surplus has been used")
